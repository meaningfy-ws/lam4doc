...
%- import "common/renderers.tex" as renderers

%- macro build_instance_view (instance_uri, parameters, level, is_concept=false)
    \VAR{ _build_instance_header_part(instance_uri, parameters, level, is_concept=is_concept) }
    \VAR{ _build_instance_metadata_part(instance_uri, parameters) }
    \VAR{ _build_instance_editorial_part(instance_uri, parameters) }
    \VAR{ _build_instance_structural_relations_part(instance_uri, parameters) }
%- endmacro
...
%- macro _build_instance_metadata_part(instance_uri, parameters)
    ...
    %- call renderers.render_fetch_results(content, error) 
        %- if not content.dropna(how="all").empty 
            ...
            \VAR{ renderers.render_description_item_plain_literal (content, "notation", "Notation") }
            \VAR{ renderers.render_description_item_plain_literal (content, "identifier", "Identifier") }
            \VAR{ renderers.render_description_item_plain_literal (content, "order", "Order") }
            \VAR{ renderers.render_description_item_plain_literal (content, "class", "Class") }
            \VAR{ renderers.render_description_item_plain_literal (content, "path", "Path") }
        %- endif
    %- endcall 
%- endmacro
 ...
 \BLOCK{ set select_query =
    ...
    %- call renderers.render_fetch_results(content, error)
        %- if not content.dropna(how="all").empty
            ...
            \VAR{ renderers.render_description_item_local_iri (content, "broaderConcept", "Broader concepts", None) }
        %- endif
    %- endcall
%- endmacro
...
%- macro build_instance_collections_recursively(parent_collection_uri, query_substitution_parameters, level)
    %- call(member_collection) selectors.select_member_collections(parent_collection_uri, query_substitution_parameters)
        \VAR{ build_instance_view(instance_uri=member_collection, parameters=conf.lam_properties, level=level, is_concept=false) }
...
%- macro build_instance_concepts(parent_collection_uri, query_substitution_parameters, level)
    %- call(member_collection) selectors.select_member_concepts(parent_collection_uri, query_substitution_parameters)
        \VAR{ build_instance_view(instance_uri=member_collection, parameters=conf.lam_properties, level=level, is_concept=true) }
    %- endcall
%- endmacro