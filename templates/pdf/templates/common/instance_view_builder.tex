...
%- macro build_lam_classification_hint_or_property_configuration(parameters, instance_uri)
 \BLOCK{ set select_query =
    ...
    FROM <~graph>
    {
     ...
     %- call macros.render_fetch_results(content, error)
     \VAR { content|string }
     \VAR { render_lam_classification_hint_or_property_configuration(content.dropna(how="all")) }

        \BLOCK { for idx, item in content.iteritems() }
            \VAR{ build_lam_classification_hint_or_property_configuration(parameters, item) }
        \BLOCK{ endfor }
     %- endcall
%- endmacro
...
%- macro render_lam_classification_hint_or_property_configuration(content)
    \BLOCK { if not content.empty }
        \VAR { macros.render_inline_link(content, 'path', 'property') }
        \VAR { macros.render_inline_values(content, 'minCount', 'min', '0') }
        \VAR { macros.render_inline_values(content, 'maxCount', 'max', '*') }
        \VAR { macros.render_inline_values(content, "flags", "flags") }
        \VAR { macros.render_inline_values(content, "pattern", "pattern") }
        \VAR { macros.render_inline_values(content, "class", "class") }
        \VAR { macros.render_inline_values(content, "value", "value") }
    \BLOCK { endif }
%- endmacro

%- macro build_instance_property_configurations_part(parameters, instance_uri)
 \BLOCK{ set select_query =
    """PREFIX sh: <http://www.w3.org/ns/shacl#>
    PREFIX lam: <http://publications.europa.eu/ontology/lam-skos-ap#>
    ...
    %- call macros.render_fetch_results(content, error)
        %- if not content.dropna(how="all").empty
     \VAR { content|string }
    \BLOCK { for row in content.itertuples() }
     \VAR { build_lam_classification_hint_or_property_configuration(parameters, row.propertyConfiguration) }
    \BLOCK { endfor }
